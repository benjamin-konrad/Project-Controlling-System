...
public class bigquerytest extends HttpServlet {
	...
	private static final Logger log = Logger.getLogger(bigquerytest.class
			.getName());
	 ...
	 * 
	 * 
	 * // ENTER YOUR PROJECT ID HERE private static final String PROJECT_NUMBER
	 * = "480761361715";
	 * 
	 * private static final String BIGQUERY_SCOPE =
	 * "https://www.googleapis.com/auth/bigquery";
	 * 
	 * AppIdentityCredential credential = new
	 * AppIdentityCredential(BIGQUERY_SCOPE); Bigquery bigquery =
	 * Bigquery.builder(TRANSPORT,JSON_FACTORY)
	 * .setHttpRequestInitializer(credential) .setJsonHttpRequestInitializer(new
	 * JsonHttpRequestInitializer() { public void initialize(JsonHttpRequest
	 * request) { BigqueryRequest bigqueryRequest = (BigqueryRequest) request;
	 * bigqueryRequest.setPrettyPrint(true); } }).build();
	 * 
	 * public void doGet(HttpServletRequest req, HttpServletResponse resp)
	 * throws IOException { resp.setContentType("text/plain");
	 * resp.getWriter().println(bigquery.datasets() .list(PROJECT_NUMBER)
	 * .execute().toString()); }
	 */// ///////////////////////////////////////////////////////////////////////////
	/**
	 * @param args
	...
	public static void main(String[] args) throws IOException,
		...
		// Print out available datasets in the "publicdata" project to the
		// console
	...
	public static GetQueryResultsResponse queryBig(String args, String param) {
		...
		if (args.equals("AllYears"))
			...
			querySql = "SELECT SUBSTR(monat, 1, 2) as	quart FROM 480761361715:csv_data.data where monat contains \""
					+ param + "\" group by quart";
			...
			querySql = "SELECT projekt FROM 480761361715:csv_data.data where bereich=\""
					+ param + "\" group by projekt";
		else if (args.equals("Konten"))
			querySql = "SELECT konto FROM 480761361715:csv_data.data where bereich=\""
					+ param.split(",")[0]
					+ "\" and projekt=\""
					+ param.split(",")[1] + "\" group by konto";
			...
			querySql = "SELECT mitarbeiter FROM 480761361715:csv_data.data where entwicklungsstufe="
					+ param + " group by mitarbeiter";
		else
			querySql = param;
		JobReference jobId = startQuery(bigquery, PROJECT_ID, querySql);
	...
	private static GetQueryResultsResponse returnresult(Bigquery bigquery,
	...
	private static Job checkQueryResults(Bigquery bigquery, String projectId,
		...
		while (true) {
			Job pollJob;
			try {
				pollJob = bigquery.jobs().get(projectId, jobId.getJobId())
						.execute();
	 ...
	 * 
	 * @param clientSecretsLocation
	 *            a path to a client_secrets.json file
